...
class PeripheralTestWindow(QDialog):
    def __init__(self, parent):
        ...
        self.name_dict = {"SSD":"SSD", "Mini SSD":"Mini SSD", 'UFS':'UFS', 'TF':'TF Card', 'FlashDrive':'U盘'}

        self.th_dict = {'SSD':380*0.25, 'Mini SSD':380*0.25, 'UFS':380*0.25, 'TF':40.0*0.20,'FlashDrive':50.0*0.25} # key corresponds to name
        ...
        self.test_sequence.append({"type":"storage", "name": "Mini SSD",    "control":self.mini_ssd_control,    "output_file":'/dev/nvme1n1',   "seek":0})
        self.test_sequence.append({"type":"storage", "name": "UFS",         "control":self.ufs_control,         "output_file":'/dev/sda',       "seek":10240})
        self.test_sequence.append({"type":"storage", "name": "TF",          "control":self.tf_card_control,     "output_file":'/dev/mmcblk0',   "seek":0})
    ...
    def initUI(self):
        ...
        # Mini SSD
        mini_ssd_frame, self.mini_ssd_control = self.gen_storage_device_testing_ui(self.name_dict['Mini SSD'], font_size=font_size)
        layout.addWidget(mini_ssd_frame)
    ...
    def find_sdb_to_sdf_devices(self):
        try:
            file_list = ['/dev/'+sub for sub in ['sdb', 'sdc', 'sdd', 'sde', 'sdf']]
